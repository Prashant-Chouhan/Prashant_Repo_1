...
function View(bindActions, model, actions, options) {
  ...
  this.lazy_children = {}
  this._listeners = {}
  this.nodeMap = getNodeData();
  this.newNode = null
  this.attachListeners()
  
}

// ***** Basic Node data Map Struct **
function getNodeData(rootId , content ,children){
	
	if(arguments.length == 0){
	return {'pRoot' : {'content' : getParentRootContent() , 'children' : {}} };
    }
return {rootId : {'content' : content , 'children' : {}} };

} 

function addChildToChildren(child){
	(this.nodeMap[rootId])['children'][] = children;
}

function removeChild(position){
	(this.nodeMap[rootId])['children'].splice(position,1) 
}
	

function removeContent(position){
	(this.nodeMap[rootId])['content'].splice(position,1) 
}
...
function setContent(content){
	(this.nodeMap[rootId])['content'] = content;
}

function getChildrenLength(){
	return (this.nodeMap[rootId])['children'].length
}

function getContentLength(){
	return (this.nodeMap[rootId])['content'].length
}

function getParentRootContent(){
	  ...
	  xmlhttp.open("GET","getParentRootContent.view",true);
	  xmlhttp.send();
}
